...
>
> Paste this whole document to your build agent as the brief for building the app. It specifies the
> mission, the non-negotiable principles, the architecture, the data model (read side), the inference
> engine, the UI, the design system, and the test philosophy. Follow the **INVARIANTS** in §0 exactly
> (they are non-negotiable); treat the formulas, thresholds, colour tokens, layout and implementation
> details as **REFERENCE** you may improve on per §0.

---

## 0. How to read this brief — latitude & invariants (READ FIRST)

You are (this variant assumes) a **more capable model** than the one that first built this tool. This
brief is detailed on purpose, but its details are of **two kinds**, and you must treat them differently.

- **INVARIANTS — binding; do not relax them for cleverness.** All of **§2** (offline-first;
  provenance-first, every shown value sourced; **no gene ever special-cased**; area membership is 100%
  **data-driven**; falsifiable tests; honest-about-limits; human voice, no spaced em dash in user copy).
  The **dual-hub symmetry** (§1A/§6). **"Sourced, never invented."** The reproducibility-critical exact
  strings: the **byte-identical** Europe PMC co-mention queries (§8) and the exact ClinVar `[Filter]`
  tokens (§5), so the app's links reproduce the pipeline's counts. The **falsifiable test philosophy**
  of **§9** (assert generic invariants, never a pinned biological conclusion). The researcher-directed
  **BioGRID** filters (§8: human, physical, yeast-two-hybrid excluded). These make the tool a
  *trustworthy research instrument*, not a plausible-looking demo. A stronger model must honour them
  **harder**, not less: cleverness is exactly what makes results subtly wrong yet convincing. When in
  doubt, keep the invariant.

- **REFERENCE — a current, working choice; improve it if you genuinely can.** The design-system values
  (**§10**: exact hexes, geometry, the chip recipe), the composite **weights and formula** (**§6.1**,
  already labelled an editorial choice), the specific **layout / component structure** and interaction
  solutions (**§7 / §7A**), and any implementation note tied to the original vanilla build (the
  CSS-specificity fix, the `16px`-input / `100dvh` / custom-autocomplete notes). These are a *baseline,
  not a target.* A stronger visual language, a clearer UX, a better prioritisation scheme, a better
  implementation — all welcome, **provided** you (a) preserve every INVARIANT, (b) keep membership
  provably data-driven, (c) keep the falsifiable tests passing or make them **stricter**, and (d)
  document what you changed and why.

**The tests are the contract.** `data/verify.js` + `data/verify_data.js` define what must be *true*; how
you get there is yours. You may re-architect freely — even change the stack or the whole scoring scheme —
as long as those invariants hold. The **anti-bias test pins no outcome**: it checks that the engine's
membership **equals an independent recomputation from the raw data**, so you may change thresholds,
weights, or the scoring entirely **as long as the engine and the test compute the same rule and every
value stays sourced**.

**Goals before solutions.** Where this brief gives a specific recipe (a colour, a pixel, a formula, a DOM
shape), read it as "one way that works." Honour the stated *goal*; the recipe is the floor, not the ceiling.

**The one stack property that must survive:** the tool **opens and runs offline from a `file://` page
(or a static host) with no network at load and no build step required to run.** The original uses vanilla
HTML/CSS/JS to guarantee this; use **any** approach that preserves it (a no-build library, or committed
build output that still opens from `file://`). Do not adopt anything that needs a build step or a live
server merely to view the page.

Everything below is the original brief; read it through this lens.
