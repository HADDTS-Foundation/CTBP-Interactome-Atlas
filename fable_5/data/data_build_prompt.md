...
>
> Paste this whole document to your build agent as the brief for building the data pipeline. Follow the
> **INVARIANTS** in §0 exactly (the endpoints, tokens and query rules are reproducibility-critical);
> treat thresholds, the step breakdown and the fetch mechanics as **REFERENCE** you may improve per §0.

---

## 0. How to read this brief — latitude & invariants (READ FIRST)

Same two-kinds rule as the app brief (`app_build_prompt.md` §0). On the data side:

- **INVARIANTS — strict.** **Standard library only, key-free, no invented values.** The exact public
  endpoints and, above all, the **byte-identical** Europe PMC co-mention queries and the exact ClinVar
  `[Filter]` tokens (§6) — so the app's in-page links reproduce these counts. The **dual-hub symmetry**.
  The researcher-directed **BioGRID** filters (human, physical, yeast-two-hybrid excluded). **Graceful
  degradation** and "**leave `null` rather than guess**." The data-integrity self-tests (§8). These are
  what make the snapshot trustworthy and reproducible — keep them, and honour them harder if you are a
  stronger model.

- **REFERENCE — engineering choices; improve freely.** The step breakdown and file layout, the
  caching / retry / rate-limit mechanics, and the tuning constants (top-N, sleeps, the area/floor
  thresholds). If a better source or method — **still key-free, still fully sourced** — yields more
  complete data, use it, **provided** the emitted `app-data.js` still satisfies §8 here and the app
  brief's §4 read-side contract, and **every value stays sourced**. Never trade provenance or
  reproducibility for coverage.

**The tests are the contract:** `data/verify_data.js` (data-only) and `data/verify.js` (with the engine)
define what must be true of the snapshot; how you fetch and assemble it is yours.

Everything below is the original data brief; read it through this lens.
